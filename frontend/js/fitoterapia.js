...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // O resto do arquivo (populateFilters, applyFilters, etc.)
    // continua exatamente igual, pois a lógica de UI não muda.

    function populateFilters(fitos) { /* ...código existente sem alterações... */ }
    function applyFilters() { /* ...código existente sem alterações... */ }
    function renderFitos(fitosToRender) { /* ...código existente sem alterações... */ }
    ...
    function renderFitos(fitosToRender) { listContainer.innerHTML = ''; if (fitosToRender.length === 0) { listContainer.innerHTML = '<p class="text-center text-muted">Nenhum fitoterápico encontrado com os filtros aplicados.</p>'; return; } fitosToRender.forEach(fitoItem => { const card = document.createElement('div'); card.className = 'col-md-6 col-lg-4 mb-4'; card.innerHTML = ` <div class="card h-100"> <div class="card-body"> <h5 class="card-title">${fitoItem.propertyName}</h5> <h6 class="card-subtitle mb-2 text-muted">${fitoItem.diseaseTreatedName}</h6> <span class="badge bg-secondary">${fitoItem.popularName}</span> </div> <div class="card-footer bg-white d-flex justify-content-end"> <a href="fitoterapia-form.html?id=${fitoItem.id}" class="btn btn-sm btn-outline-secondary me-2">Editar</a> <a href="#" class="btn btn-sm btn-outline-primary details-btn" data-id="${fitoItem.id}" data-bs-toggle="modal" data-bs-target="#fitoDetailsModal">Ver Detalhes</a> </div> </div> `; listContainer.appendChild(card); }); setupModalButtons(); }
    function setupModalButtons() { const modalTitle = document.getElementById('modalTitle'); const modalBody = document.getElementById('modalBody'); document.querySelectorAll('.details-btn').forEach(button => { button.addEventListener('click', (event) => { const fitoId = event.target.getAttribute('data-id'); const fitoItem = allFitos.find(f => f.id == fitoId); if (fitoItem) { modalTitle.textContent = fitoItem.popularName; let imageHtml = ''; if (fitoItem.imageUrl && fitoItem.imageUrl.toLowerCase() !== 'n/h') { imageHtml = `<img src="${fitoItem.imageUrl}" alt="Imagem de ${fitoItem.popularName}" class="img-fluid rounded mb-3">`; } modalBody.innerHTML = ` ${imageHtml} <p><strong>Nome Científico:</strong> <em>${fitoItem.scientificName}</em></p> <hr> <h5>Método de Preparo</h5> <p>${fitoItem.preparationMethod.replace(/\n/g, '<br>')}</p> <h5>Contraindicação</h5> <p>${fitoItem.contraindication.replace(/\n/g, '<br>')}</p> `; } }); }); }
